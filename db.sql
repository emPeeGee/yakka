...
-- Create a policy on the 'words' table to restrict access to admins
CREATE POLICY daily_words_admin_only_policy ON daily_words
    FOR ALL
    TO admin_role
    USING (true); -- Allow access for admins

CREATE POLICY daily_words_read_only_policy ON daily_words
    FOR SELECT
    TO public
    USING (true);  -- Allow SELECT for everyone
...
-- Create a policy on the 'words' table to restrict access to admins
CREATE POLICY lessons_admin_only_policy ON lessons
    FOR ALL
    TO admin_role
    USING (true); -- Allow access for admins

CREATE POLICY lessons_read_only_policy ON lessons
    FOR SELECT
    TO public
    USING (true);  -- Allow SELECT for everyone
